...
(function () {
    ...
    // 统计某个听牌 Hand 的好形率
    function computeKoukeiRate(hand) {
        if (!hand.children || hand.children.length === 0) return 0;
        const total = getTotalTileCounts(hand.children);
        const koukei = hand.children.filter(h => getTotalTileCounts(h.children) > 4)
            .reduce((a, h) => a + getTotalTileCounts(h.children), 0);
        return total > 0 ? koukei / total : 0;
    }

    // 获取某个切牌（进入 1-shanten）下所有听牌路径中最高的好形率
    function getMaxKoukeiRateForDiscard(hand) {
        let maxRate = 0;
        for (const child of hand.children) { // 每个 child 是进入听牌的1-shanten手牌
            const rate = computeKoukeiRate(child);
            if (rate > maxRate) maxRate = rate;
        }
        return maxRate;
    }

    // 获取2-shanten下所有切牌平均最大好形率（全局评价）
    function getAverageKoukeiRateFor2Shanten(hand) {
        const rates = hand.children.map(getMaxKoukeiRateForDiscard);
    ...
    function getShantenRow(config) {
        ...
        if (typeof config.koukeiRate === "number") {
            tdData.push([`（好形率${Math.round(config.koukeiRate * 100)}%）`]);
        }
    ...
    const isKoukeiFor2Shanten = (hand) => {
        let totalKoukei = 0;
        for (const child1 of hand.children) { // 每个child是1-shanten手牌
            if (!child1.children || child1.children.length === 0) continue;
            for (const child2 of child1.children) { // 每个child2是听牌手牌
                const count = getTotalTileCounts(child2.children);
                if (count > 4) {
                    totalKoukei += count;
                }
            }
        }
        return totalKoukei > 4;
    };
    ...
    const run = () => {
        ...
        if (currentShanten === 2) {
            const avgRate = getAverageKoukeiRateFor2Shanten(hand);
            const avgDiv = document.createElement("div");
            avgDiv.style.marginTop = "10px";
            avgDiv.style.fontWeight = "bold";
            avgDiv.innerText = `平均好形率: ${(avgRate * 100).toFixed(1)}%`;
            originalTable.parentNode.insertBefore(avgDiv, originalTable.nextSibling);
        }
